...
def _non_saturating_loss(D_real_logits, D_gen_logits):
...
def _least_squares_loss(D_real, D_gen):
...
def gan_loss(gan_loss_type, disc_out, mode='generator_loss'):
    if gan_loss_type == 'non_saturating':
        ...
        D_loss, G_loss = loss_fn(D_real_logits=disc_out.D_real_logits, D_gen_logits=disc_out.D_gen)
