...
def load_model(save_path, logger, device, model_type=None, model_mode=None, current_args_d=None, prediction=True,
    ...
    from src.models.hific.model import Model
    ...
    if current_args_d is not None:
        if silent is False:
            for k, v in current_args_d.items():
                ...
                if loaded_v != v:
                    logger.warning(
                        'Current argument {} (value {}) does not match recorded argument (value {}). Recorded '
                        'argument will be overriden.'.format(
                            k, v, loaded_v))
...
def log(model, storage, epoch, idx, mean_epoch_loss, current_loss, best_loss, start_time, epoch_start_time,
    ...
    if header == '[TRAIN]':
        ...
        report_f("Epoch {} | Mean epoch comp. loss: {:.3f} | Current comp. loss: {:.3f} | Improved: {}".format(
            epoch,
            mean_epoch_loss,
            current_loss,
            improved))
    ...
    report_f(
        "avg. original bpp: {:.3f} | n_bpp (total): {:.3f} | q_bpp (total): {:.3f} | n_bpp (latent): {:.3f} | q_bpp ("
        "latent): {:.3f} | ""n_bpp (hyp-latent): {:.3f} | q_bpp (hyp-latent): {:.3f}".format(
            avg_bpp, storage['n_rate'][-1],
            storage['q_rate'][-1],
