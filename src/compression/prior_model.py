...
# Custom
from src.helpers import math_material
...
class PriorEntropyModel(entropy_models.ContinuousEntropyModel):
    ...
    def build_tables(self, **kwargs):
        ...
        for n, (pmf_, pmf_length_, tail_) in enumerate((zip(tqdm(pmf), pmf_length, tail_mass))):
            ...
            cdf_ = math_material.pmf_to_quantized_cdf(pmf_, self.precision)
...
class PriorDensity(nn.Module):
    ...
    def __init__(self, n_channels, min_likelihood=MIN_LIKELIHOOD, max_likelihood=MAX_LIKELIHOOD,
        ...
        if likelihood_type == 'gaussian':
            ...
            self.standardized_quantile = math_material.standardized_quantile_gaussian
            self.quantile = math_material.quantile_gaussian

        elif likelihood_type == 'logistic':
            self.standardized_CDF = math_material.standardized_CDF_logistic
            self.standardized_quantile = math_material.standardized_quantile_logistic
            self.quantile = math_material.quantile_logistic
